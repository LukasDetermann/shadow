...
If configured explicitly in any way it works as expected.
How auto-detection works is a longer story.

== Java Compiler

*Pseudocode of the Java Compiler (javac) in Jdk 23*
...
----
<3> `+-processor+`
With this command line Option a List of Annotation processors can be set
+
[,bash]
----
...
* Per default maven docent use the dependency management section to resolve transitive dependencies in Annotation Processors
...
https://issues.apache.org/jira/projects/MCOMPILER/issues/MCOMPILER-97[javac will try to compile the Processor using the Processor.]
...
* Before each round, except the last, javac tries to find more
