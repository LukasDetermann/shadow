                                     ...
                                     Enum
{
   ...
   {
      return stream(getaClass().getDeclaredFields()).map(ReflectionAdapter::getShadow).collect(Collectors.toUnmodifiableList());
   }

   @Override
   public List<Method> getMethods()
   {
      return stream(getaClass().getDeclaredMethods()).map(ReflectionAdapter::getShadow).collect(Collectors.toUnmodifiableList());
   }

   @Override
   public List<Constructor> getConstructors()
   {
      return stream(getaClass().getDeclaredConstructors()).map(ReflectionAdapter::getShadow).collect(Collectors.toUnmodifiableList());
   }
