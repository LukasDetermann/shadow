...
class StringUtils
{
   ...
   private static final Predicate<String> SCREAMING_SNAKE_CASE = s -> Pattern.compile("^[A-Z][A-Z0-9]*(_[A-Z0-9]+)*$").matcher(s).matches();
   private static final Predicate<String> LOWER_CAMEL_CASE = s -> Pattern.compile("^[a-z][a-zA-Z0-9]*$").matcher(s).matches();
   private static final Predicate<String> UPPER_CAMEL_CASE = s -> Pattern.compile("^[A-Z][a-zA-Z0-9]*$").matcher(s).matches();
