...
public class InterfaceImpl extends DeclaredImpl implements Interface
{
   ...
   {
      return Arrays.stream(getaClass().getTypeParameters()).map(ReflectionAdapter::getShadow).map(Generic.class::cast).collect(collectingAndThen(toList(), Collections::unmodifiableList));
   }
