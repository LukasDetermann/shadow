...
class PropertyTest
{
   ...
   void id()
   {
      withSource("Pojo", """
            ...
            .test(implementation ->
                  {
                     C.Class cClass = requestOrThrow(implementation, GET_CLASS, "Pojo");
                     List<? extends C.Property> properties = requestOrThrow(cClass, CLASS_GET_PROPERTIES);
                     ...
                     C.Nameable type = ((C.Nameable) requestOrThrow(properties.get(1), PROPERTY_GET_TYPE));
                     assertEquals("Long", requestOrThrow(type, NAMEABLE_GET_NAME));
                     C.Field field = requestOrThrow(properties.get(1), PROPERTY_GET_FIELD);
                     assertEquals("id", requestOrThrow(field, NAMEABLE_GET_NAME));
                     C.Method getter = requestOrThrow(properties.get(1), PROPERTY_GET_GETTER);
                     assertEquals("getId", requestOrThrow(getter, NAMEABLE_GET_NAME));
                     C.Method setter = requestOrThrow(properties.get(1), PROPERTY_GET_SETTER);
   ...
   void overwrittenAccessor()
   {
      withSource("Parent", "abstract class Parent{public abstract Long getId();}")
            ...
            .test(implementation ->
                  {
                     C.Class cClass = requestOrThrow(implementation, GET_CLASS, "Child");
                     List<? extends C.Property> properties = requestOrThrow(cClass, CLASS_GET_PROPERTIES);
                     ...
                     C.Method getter = requestOrThrow(properties.get(1), PROPERTY_GET_GETTER);
                     C.Declared cDeclared = requestOrThrow(getter, EXECUTABLE_GET_SURROUNDING);
            ...
            .test(implementation ->
                  {
                     ...
                     C.QualifiedNameable fieldType = ((C.QualifiedNameable) requestOrThrow(field, VARIABLE_GET_TYPE));
